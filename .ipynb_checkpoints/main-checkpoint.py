...
class MainWindow(QMainWindow):
    ...
    def training_end_slot(self, msg):
        ...
        text = self.ui.tb_train_result.toPlainText()  # 获得原本显示的文字
        # PyTorch返回的score是列表[loss, acc]，随机森林返回的是数字
        if isinstance(self.score, list):
            score_text = f'Loss: {self.score[0]:.4f}, Accuracy: {self.score[1]:.4f}'
        else:
            score_text = str(self.score)
        self.ui.tb_train_result.setText(text + '\n训练完成，模型得分：' + score_text + '\n--------------')
    ...
    def show_result(self):
        ...
        if -2 == show_mode:  # 展示 分类报告
            ...
            # 读取图片文件，进行显示
            img_path = self.cache_path + '/' + self.model_name + '_confusion_matrix.png'
            if not os.path.exists(img_path):
                QMessageBox.warning(self, '提示', f'图片文件不存在：{img_path}', QMessageBox.Yes)
                return
            img = QImage(img_path)
            if img.isNull():
                QMessageBox.warning(self, '提示', '图片加载失败！', QMessageBox.Yes)
                return
            img_result = img.scaled(self.ui.l_train_result.width(), self.ui.l_train_result.height(),
                                    Qt.KeepAspectRatio, Qt.SmoothTransformation)
            self.ui.l_train_result.setPixmap(QPixmap.fromImage(img_result))
        elif -4 == show_mode:  # 展示 ROC曲线
            # 读取图片文件，进行显示
            img_path = self.cache_path + '/' + self.model_name + '_ROC_Curves.png'
            if not os.path.exists(img_path):
                QMessageBox.warning(self, '提示', f'图片文件不存在：{img_path}', QMessageBox.Yes)
                return
            img = QImage(img_path)
            ...
            # 读取图片文件，进行显示
            img_path = self.cache_path + '/' + self.model_name + '_Precision_Recall_Curves.png'
            if not os.path.exists(img_path):
                QMessageBox.warning(self, '提示', f'图片文件不存在：{img_path}', QMessageBox.Yes)
                return
            img = QImage(img_path)
            ...
            if 'random_forest' == self.model_name:  # 随机森林没有损失曲线
                ...
                # 读取图片文件，进行显示
                img_path = self.cache_path + '/' + self.model_name + '_train_valid_loss.png'
                if not os.path.exists(img_path):
                    QMessageBox.warning(self, '提示', f'图片文件不存在：{img_path}', QMessageBox.Yes)
                    return
                img = QImage(img_path)
            ...
            if 'random_forest' == self.model_name:  # 随机森林没有正确率曲线
                ...
                # 读取图片文件，进行显示
                img_path = self.cache_path + '/' + self.model_name + '_train_valid_acc.png'
                if not os.path.exists(img_path):
                    QMessageBox.warning(self, '提示', f'图片文件不存在：{img_path}', QMessageBox.Yes)
                    return
                img = QImage(img_path)
    ...
    def save_model(self):
        ...
        if 'random_forest' == self.model_name:
            ...
            save_path, _ = QFileDialog.getSaveFileName(self, '保存文件', './' + self.model_name + '.pth', '(*.pth)')
            if '' == save_path:  # 没有确定保存。这里也可以通过 变量 _ 来判断
                return

            torch.save(self.model, save_path)  # 保存整个模型
            md5 = generate_md5(save_path)
            model_config = {'mean': self.scaler_info['mean'], 'std': self.scaler_info['std'], 'md5': md5}
            with open(fr'{save_path[: -4]}.json', 'w') as jf:
                json.dump(model_config, jf)
    ...
    def select_model(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(self, '选择模型', '.', '(*.m *.pth *.pt)')
        if '' != file_path:  # 选择了文件, 则将路径更新，否则，保留原路径
...
def CNN_1D_training(data_path, signal_length, signal_number, normal, rate, save_path, model_name, training_end_signal):
    ...
    msg = {'model': model, 'classification_report': classification_report, 'score': score,
